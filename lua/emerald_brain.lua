...
local function start_server()
    local ok, err = pcall(function()
        server = socket.bind("0.0.0.0", LISTEN_PORT)
        if server then
            server:listen(5)
...
local function handle_command(cmd_obj)
    ...
    elseif action == "readfile" then
        -- Read a file from disk and return its contents as hex
        local fpath = cmd_obj.path
        if not fpath then
            return {error = "readfile: missing path"}
        end
        local rf_ok, rf_result = pcall(function()
        ...
        if rf_ok and rf_result then
            -- Encode as hex string
            local hex_parts = {}
            for i = 1, #rf_result do
                hex_parts[i] = string.format("%02x", rf_result:byte(i))
            end
            return {ok = true, data = table.concat(hex_parts), size = #rf_result}
        else
            return {error = "readfile failed: " .. tostring(rf_result or "file not found")}
        end
    ...
    elseif action == "readrange" then
        local addr = cmd_obj.addr
        local length = cmd_obj.length
        if not addr then return {error = "missing 'addr'"} end
        if not length then return {error = "missing 'length'"} end
        if length > 256 then return {error = "length exceeds 256 byte limit"} end
        local hex = {}
        for i = 0, length - 1 do
            hex[#hex + 1] = string.format("%02x", emu:read8(addr + i))
        end
        return {ok = true, data = table.concat(hex)}
