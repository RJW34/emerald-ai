...
def _find_mgba_window() -> Optional[int]:
    """Find the mGBA window ID via xdotool (or MGBA_WINDOW_ID env var)."""
...
def _find_mgba_pid() -> Optional[int]:
    """Find mGBA process ID (or MGBA_PID env var)."""
...
async def press(req: PressRequest):
    ...
    for key in x11_keys:
        try:
            # First activate the window (brings it to focus)
            proc = await asyncio.create_subprocess_exec(
            ...
            # Then send the key without --window flag (real X11 input)
            proc = await asyncio.create_subprocess_exec(
                "xdotool", "key", key,
                env=XDOTOOL_ENV,
                stdout=asyncio.subprocess.PIPE,
